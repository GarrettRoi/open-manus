...
# ---------------------------------------------------------------------------
# Vault admin proxy — lets the fleet dashboard manage vault connections
# (OAuth accounts, API keys) and per-agent grants without exposing the vault
# admin password to the browser. Requires on this service:
#   VAULT_URL          e.g. http://vault.railway.internal:8080
#   VAULT_ADMIN_TOKEN  shared secret (or VAULT_ADMIN_PASSWORD as fallback)
# Raw credentials never pass through here: the vault only ever returns
# secret-free connection views.
# ---------------------------------------------------------------------------

def _vault_admin_config() -> Tuple[str, str]:
    base = (os.getenv("VAULT_URL") or "").strip().rstrip("/")
    token = (os.getenv("VAULT_ADMIN_TOKEN") or os.getenv("VAULT_ADMIN_PASSWORD") or "").strip()
    return base, token


async def _vault_admin_call(method: str, path: str, body: Optional[dict] = None):
    import httpx
    base, token = _vault_admin_config()
    if not base or not token:
        raise HTTPException(
            status_code=503,
            detail="Vault is not configured on this dashboard. Set VAULT_URL and "
                   "VAULT_ADMIN_TOKEN environment variables on this service.")
    headers = {"X-Vault-Admin-Token": token}
    if body is not None:
        headers["Content-Type"] = "application/json"
    try:
        ...
        raise HTTPException(status_code=502, detail=f"Vault unreachable: {exc}")
    if resp.status_code >= 400:
        try:
...
async def vault_overview():
    return await _vault_admin_call("GET", "/api/admin/overview")
...
@app.post("/api/vault/connections")
async def vault_create_connection(body: VaultConnectionCreate):
    payload = {k: v for k, v in body.model_dump().items() if v is not None}
    return await _vault_admin_call("POST", "/api/admin/connections", payload)


@app.post("/api/vault/connections/{conn_id}/delete")
async def vault_delete_connection(conn_id: str):
    safe = urllib.parse.quote(conn_id, safe="")
    return await _vault_admin_call("POST", f"/api/admin/connections/{safe}/delete", {})


@app.post("/api/vault/connections/{conn_id}/connect-link")
async def vault_connect_link(conn_id: str):
    safe = urllib.parse.quote(conn_id, safe="")
...
@app.post("/api/vault/grants")
async def vault_set_grant(body: VaultGrantUpdate):
    return await _vault_admin_call("POST", "/api/admin/grants", body.model_dump())
