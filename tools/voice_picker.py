...
Used by the ``/voice-character`` command. The picker
...
def resolve_voice(selector: str) -> Dict[str, Any]:
    ...
    raise ValueError(
        f"Unknown voice {selector!r} — use /voice-character to see the list, "
...
def set_voice(selector: str) -> Dict[str, Any]:
    ...
    # atomically (temp file + rename) so concurrent /voice-character calls or
    # other config writers can't interleave or leave a torn file.
    lock_fh = open(lock_path, "w")
    try:
        if fcntl is not None:
            fcntl.flock(lock_fh, fcntl.LOCK_EX)
        # Fail closed: never replace an existing-but-unreadable config.yaml
        # with a config containing only the edited section (matches the
        # project-wide atomic_config_write invariant).
...
def format_voice_list() -> str:
    ...
    lines = ["🎙️ **Top adult female ElevenLabs voices** — swap with `/voice-character <number|name|id>`", ""]
