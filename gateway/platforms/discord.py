...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="personality", description="Set, list, or edit a personality")
        @discord.app_commands.describe(name="Personality name, 'status', or 'edit <name> <instructions>'. Leave empty to list available.")
        ...
        async def slash_nsfw(interaction: discord.Interaction, toggle: str = ""):
            ...
            async def _run_personality_command(personality_name: str) -> str:
                # Call the message handler directly (instead of
                # self.handle_message, which fires-and-forgets into the normal
                # channel-reply pipeline) so we get the handler's actual
                # success/failure text back and can surface it verbatim,
                # rather than always claiming success regardless of outcome.
                event = self._build_slash_event(interaction, f"/personality {personality_name}")
                try:
                    response = await self._message_handler(event)
                except Exception as e:
                    logger.debug("nsfw personality dispatch failed: %s", e)
                    return f"\u26a0\ufe0f Failed to switch personality: {e}"
                text, _ = self._unwrap_ephemeral(response)
                return text or "Done."

            if toggle in ("on", "yes", "true", "enable", "1"):
                result_text = await _run_personality_command("unfiltered")
                failed = result_text.strip().startswith(("\u26a0", "Unknown personality"))
                header = "\u26a0\ufe0f NSFW mode **NOT** enabled:" if failed else "\U0001f525 NSFW mode **enabled** — unfiltered personality active."
                footer = "" if failed else "\n_Use `/nsfw off` to return to normal._"
                try:
                    await interaction.followup.send(f"{header}\n{result_text}{footer}", ephemeral=True)
                except Exception as e:
                    logger.debug("Discord followup failed: %s", e)
            elif toggle in ("off", "no", "false", "disable", "0"):
                result_text = await _run_personality_command("normal")
                failed = result_text.strip().startswith(("\u26a0", "Unknown personality"))
                header = "\u26a0\ufe0f NSFW mode **NOT** disabled:" if failed else "\u2705 NSFW mode **disabled** — normal personality restored."
                ...
                # Report the agent's ACTUAL current mode instead of static help text.
                try:
                    from gateway.run import _load_gateway_config
                    from hermes_cli.config import cfg_get
                    cfg = _load_gateway_config()
                    active = str(cfg_get(cfg, "agent", "active_personality", default="") or "").strip().lower()
                except Exception as e:
                    logger.debug("Could not resolve current nsfw state: %s", e)
                    active = ""
                is_on = active == "unfiltered"
                state_line = "\U0001f525 NSFW mode is currently **ON** (unfiltered)." if is_on else "\u2705 NSFW mode is currently **OFF** (normal)."
                try:
                    await interaction.followup.send(
                        f"**NSFW Toggle**\n\n{state_line}\n\n"
                        "`/nsfw on` — Enable unfiltered mode (no content restrictions)\n"
                        "`/nsfw off` — Return to normal professional mode\n\n"
                        "_This sets the agent's personality. Use `/personality` to see all options, "
                        "or `/personality edit <name> <instructions>` to edit one._",
                        ephemeral=True
                    )
                except Exception as e:
                    logger.debug("Discord followup failed: %s", e)
