...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        """Handle /personality command - list, set, report, or edit a personality.

        Every read AND write in this handler goes through ``_gateway_config_home()``
        (via ``_load_gateway_config`` for reads, and the same helper for the write
        path) so the command always targets the responding agent's own
        ``config.yaml`` — never a stale module-level home captured at import time.
        This also matters for ``/nsfw``, which delegates to this handler under the
        hood (``/personality unfiltered`` / ``/personality normal``).
        """
        from gateway.run import _gateway_config_home, _load_gateway_config
        from hermes_constants import display_hermes_home

        # Preserve original case for edit instructions; only the leading
        # subcommand/name token is matched case-insensitively.
        raw_args = event.get_command_args().strip()
        args = raw_args.lower()
        ...
        active = str(cfg_get(config, "agent", "active_personality", default="") or "").strip().lower()
        ...
        # --- /personality edit <name> <instructions...> ---------------------
        if args.startswith("edit"):
            rest = raw_args[len("edit"):].strip()
            parts = rest.split(None, 1)
            if len(parts) < 2 or not parts[1].strip():
                return t("gateway.personality.edit_usage")

            edit_name, new_instructions = parts[0].strip().lower(), parts[1].strip()
            if edit_name not in personalities:
                available = "`none`, " + ", ".join(f"`{n}`" for n in personalities)
                return t("gateway.personality.unknown", name=edit_name, available=available)

            old_value = personalities[edit_name]
            old_prompt = _resolve_prompt(old_value)
            if isinstance(old_value, dict):
                new_value = dict(old_value)
                new_value["system_prompt"] = new_instructions
            else:
            ...
            try:
                if "agent" not in config or not isinstance(config.get("agent"), dict):
                    config["agent"] = {}
                if "personalities" not in config["agent"] or not isinstance(config["agent"].get("personalities"), dict):
                    config["agent"]["personalities"] = dict(personalities)
                config["agent"]["personalities"][edit_name] = new_value
                new_prompt = _resolve_prompt(new_value)
                # If the personality being edited is the one currently active,
                # keep the live system prompt (disk + in-memory) in sync so the
                # edit takes effect immediately rather than on the next toggle.
                if edit_name == active:
            ...
            old_preview = old_prompt[:120] + "…" if len(old_prompt) > 120 else old_prompt
            new_preview = new_instructions[:120] + "…" if len(new_instructions) > 120 else new_instructions
            return t(
                "gateway.personality.edited",
                name=edit_name,
                old=old_preview,
                new=new_preview,
            )
        ...
        if args == "status":
            ...
            return t("gateway.personality.status", name=current)

        # --- /personality (no args) — list, marking the active one ----------
        if not args:
            lines = [t("gateway.personality.header")]
            none_marker = " ✅" if not active else ""
            lines.append(t("gateway.personality.none_option") + none_marker)
            for name, prompt in personalities.items():
                ...
                lines.append(t("gateway.personality.item", name=name, preview=preview) + marker)
            lines.append(t("gateway.personality.usage"))
            lines.append(t("gateway.personality.edit_usage"))
            return "\n".join(lines)

        # --- /personality none|default|neutral -------------------------------
        if args in {"none", "default", "neutral"}:
            try:
                if "agent" not in config or not isinstance(config.get("agent"), dict):
                    config["agent"] = {}
                config["agent"]["system_prompt"] = ""
                config["agent"]["active_personality"] = "none"
                atomic_config_write(config_path, config)
            except Exception as e:
                return t("gateway.personality.save_failed", error=str(e))
        ...
        # --- /personality <name> — activate a configured personality --------
        if args in personalities:
            ...
            # keeps the ephemeral runtime prompt from drifting out of sync
